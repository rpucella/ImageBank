...
app.get('/api/image/:uuid', async (req, res) => {
  ...
  result.image = null   // not to send too much crap...
  res.json({ image: result });
});

app.get('/raw/:uuid', async (req, res) => {
  const img = await imagebank.image(_FOLDER, req.params.uuid)
...
if (process.argv.length > 3) {
  ...
  console.log(`USAGE: imagebank <db-file> <port=${DEFAULT_PORT}>`);
}
